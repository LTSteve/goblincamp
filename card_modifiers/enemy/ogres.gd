...
func _apply(melee_weapon:MeleeWeapon,_unit):
	if !GameManager.I.unlocked_enemy_types.has(UnitSpawner.UnitType.Ogre):
		GameManager.I.unlocked_enemy_types.append(UnitSpawner.UnitType.Ogre)
		GameManager.I.force_spawn_enemy_type = UnitSpawner.UnitType.Ogre
	if current_rank == 2:
...
func _un_apply(melee_weapon:MeleeWeapon,_unit):
	GameManager.I.unlocked_enemy_types = GameManager.I.unlocked_enemy_types.filter(func(enemy_type): return enemy_type != UnitSpawner.UnitType.Ogre)
